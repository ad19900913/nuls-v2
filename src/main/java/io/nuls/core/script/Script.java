...
public class Script {
    ...
    private static BigInteger castToBigInteger(byte[] chunk) throws ScriptException {
        ...
        return SerializeUtils.decodeMPI(ByteUtils.reverseBytes(chunk), false);
    }
            ...
            if (chunk.opcode == ScriptOpCodes.OP_0) {
                ...
                switch (opcode) {
                    ...
                    case ScriptOpCodes.OP_1NEGATE:
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.ONE.negate(), false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_16:
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.valueOf(decodeFromOpN(opcode)), false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_DEPTH:
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.valueOf(stack.size()), false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_SIZE:
                        ...
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.valueOf(stack.getLast().length), false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_0NOTEQUAL:
                        ...
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(numericOPnum, false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_MAX:
                        ...
                        stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(numericOPresult, false)));
                        break;
                    ...
                    case ScriptOpCodes.OP_WITHIN:
                        ...
                        if (OPWITHINnum2.compareTo(OPWITHINnum1) <= 0 && OPWITHINnum1.compareTo(OPWITHINnum3) < 0) {
                            stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.ONE, false)));
                        } else {
                            stack.add(ByteUtils.reverseBytes(SerializeUtils.encodeMPI(BigInteger.ZERO, false)));
                        }
                        break;
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            sigValid = verifySign(txContainingThis.getHash().getBytes(), sigBytes, pubKey);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                if (ECKey.verify(txContainingThis.getHash().getBytes(), sigs.getFirst(), pubKey)) {
                    sigs.pollFirst();
                }
    ...
    public String toString() {
        return CollectionUtils.join(chunks);
    }
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) {
       ...
       /* TransactionSignature signature = TransactionSignature.decodeFromBitcoin(signatureBytes, true);
        ...
        throw new IllegalStateException("Could not find matching key for signature on " + hash.toString() + " sig " + HexUtil.encode(signatureBytes));
    }
    ...
    public boolean isPayToScriptHash() {
        ...
        return program.length == 26 &&
                ...
                (program[1] & 0xff) == 0x17 &&
                (program[25] & 0xff) == ScriptOpCodes.OP_EQUAL;
    }
    ...
    public boolean isSentToMultiSig() {
        ...
        if (!(chunk.equalsOpCode(ScriptOpCodes.OP_CHECKMULTISIG) || chunk.equalsOpCode(ScriptOpCodes.OP_CHECKMULTISIGVERIFY))) {
            return false;
        }
        try {
    ...
    public boolean isSentToCLTVPaymentChannel() {
        ...
        if (!chunks.get(0).equalsOpCode(ScriptOpCodes.OP_IF)) {
            return false;
        }
        // chunk[1] = recipient pubkey
        if (!chunks.get(2).equalsOpCode(ScriptOpCodes.OP_CHECKSIGVERIFY)) {
            return false;
        }
        if (!chunks.get(3).equalsOpCode(ScriptOpCodes.OP_ELSE)) {
            return false;
        }
        // chunk[4] = locktime
        if (!chunks.get(5).equalsOpCode(ScriptOpCodes.OP_CHECKLOCKTIMEVERIFY)) {
            return false;
        }
        if (!chunks.get(6).equalsOpCode(ScriptOpCodes.OP_DROP)) {
            return false;
        }
        if (!chunks.get(7).equalsOpCode(ScriptOpCodes.OP_ENDIF)) {
            return false;
        }
        // chunk[8] = sender pubkey
        if (!chunks.get(9).equalsOpCode(ScriptOpCodes.OP_CHECKSIG)) {
            return false;
        }
        return true;
    }
